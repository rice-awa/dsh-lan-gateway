...
Touches, at minimum: the `Config` interface, the `z.object` schema and the `readConfig` unwrapping in `src/index.ts` — every field is `.volatile()`, because 0.1.7 projects only volatile fields into forms and refuses an edit to any other path — `listenerKey` if it affects the listener, `FIELDS` in `src/config-fields.ts` (which is what makes the card render it, the config route accept it, and `OPTIONAL_CONFIG_KEYS` clear it on empty), the `LanGatewaySettings` wire shape in the same file if it is card-editable, the i18n tables in `src/client/lan-gateway-card.tsx`, and the config table in README.md.
